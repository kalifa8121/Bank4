...
# Yoo Render Disk kaayyan (e.g. /var/data), folder-oota kana achitti galcha
...
def auto_backup_db():
    try:
        if os.path.exists(DB_PATH):
            ...
            # Backup dhumaa akka auto_restore_latest.db tiin kaaya
            latest_path = os.path.join(BACKUP_FOLDER, "auto_restore_latest.db")
            shutil.copyfile(backup_file_path, latest_path)
            print("💾 [AUTO BACKUP]: Database backup successfully created.")
...
def auto_restore_db():
    try:
        ...
        if not os.path.exists(DB_PATH) and os.path.exists(latest_path):
            ...
            print("🔄 [AUTO RESTORE]: Database restored from latest backup.")
    except Exception as e:
        print(f"⚠️ [AUTO RESTORE ERROR]: {e}")

# Register startup auto restore fi shutdown auto backup
auto_restore_db()
...
# --- 3. AUDITOR FI MANAGER APPROVAL WITH FULL VIEW ---
@app.route('/pending')
def pending():
...
def auditor_reversal_request():
    ...
    if request.method == 'POST':
        ...
        if not txn:
            ...
            send_notification(f"Gaaffiin Reversal haaraa ({txn_id}) AUDITOR'n ergameera.", session['username'])
        conn.close()
...
def approve_reversal(role_type, rev_id):
    ...
    if mgr_appr == 1 and ceo_appr == 1:
        ...
        if txn and txn['status'] == 'APPROVED':
            ...
            send_notification(f"Reversal ({rev['txn_id']}) guutumaan guutuutti mirkanaa'ee reversed ta'era.", session['username'])
    else:
...
# --- 8 & 9. AUTO BACKUP & RESTORE VIEW FOR CEO ---
...
def ceo_backup():
    ...
    if request.method == 'POST':
        ...
        if action == 'restore':
            ...
            if file and file.filename.endswith('.db'):
                ...
                msg_type = "red"
        elif action == 'auto_backup_now':
            auto_backup_db()
            msg = "✅ Auto-backup saafiyyaan raawwatameera!"
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size: 16px; color:#581c87; margin-bottom: 4px;">💾 Safe Data Backup & Auto Restore (CEO)</h2>
        <p style="font-size: 11px; color:#64748b; margin-bottom: 16px;">Render restart yoo ta'e database keessan otuu hin haqamin auto-restore ta'a.</p>
        
        {msg_html}

        <div style="background:#faf5ff; border:1px solid #e9d5ff; padding:16px; border-radius:10px; margin-bottom:16px;">
            <h3 style="font-size:13px; color:#581c87; margin-bottom:4px;">📥 1. Auto & Manual Backup</h3>
            <p style="font-size:11px; color:#64748b; margin-bottom:10px;">Database ammaa save godhachuuf ykn instant backup kaayuuf tuqaa.</p>
            <a href="/download_db" class="btn-submit" style="background:#7c3aed; text-align:center; text-decoration:none; display:block; margin-bottom:8px;">💾 Download Database Backup (.db)</a>
            <form method="POST">
                <input type="hidden" name="action" value="auto_backup_now">
                <button type="submit" class="btn-submit" style="background:#0284c7;">⚡ Direct System Backup Kaayi</button>
            </form>
        </div>
...
# --- 1. RECEIPT PRINTING (STATU APPROVE/PENDING OSOO HIN AGARSIISIN) ---
...
# --- 5 & 6. REGISTER WITH NATIONAL ID UPLOAD & INITIAL BALANCE DEPOSIT RECORD ---
...
def register():
    ...
    if request.method == 'POST':
        ...
        if initial_balance < 0:
            msg = "❌ Balansii jalqabaa negative (qarshii hir'aa) gochuun HIN DANDA'AMU!"
        else:
            photo_file = request.files.get('photo')
            sig_file = request.files.get('signature')
            nid_file = request.files.get('national_id')
            ...
            if photo_file and sig_file and allowed_file(photo_file.filename) and allowed_file(sig_file.filename):
                ...
                if nid_file and allowed_file(nid_file.filename):
                    nid_filename = f"nid_{timestamp_str}_" + secure_filename(nid_file.filename)
                    nid_file.save(os.path.join(app.config['UPLOAD_FOLDER'], nid_filename))
                ...
                """, (cust_id, full_name, phone, photo_filename, sig_filename, nid_filename, initial_balance, now))

                # Yoo initial balance > 0 ta'e transaction deposit uuma
                if initial_balance > 0:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, 'DEPOSIT', ?, ?, ?, 'Imana Core Initial', ?, 'PENDING_APPROVAL', ?, ?)
                ...
                # 10. NOTIFICATION SEND
                send_notification(f"Maammilli haaraa {full_name} (Acc: {cust_id}) galmaa'era.", session['username'])
                msg = f"⏳ Maammilli {full_name} (Initial Balance: {initial_balance:,.2f} Birr) galmaa'eera! (T24 Acc: {cust_id}). MANAGER/AUDITOR ACTIVE akka ta'u mirkaneessuu qaba."
    ...
    <div class="box">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>🪪 National ID (Suuraa ykn PDF Upload)</label>
...
# --- 1. MAKER TRANSACTION WITH INSTANT PRINT PREVIEW ---
...
def transaction():
    ...
    if request.method == 'POST':
        ...
        if cust_freeze == 'FROZEN' and txn_type in ['WITHDRAWAL', 'T24_TRANSFER']:
            ...
            msg = f"⏳ {txn_type} Ref: {ft_ref} ({amount:,.2f} Birr) Mirkaneessa eegaa jira!"
            send_notification(f"Kaffaltii haaraa {ft_ref} ({txn_type} {amount} Birr) MAKER'n uumameera.", session['username'])
    ...
    popup_script = f"<script>window.onload = function() {{ openReceiptModal('{created_txn_id}'); }};</script>" if created_txn_id else ""
    ...
    <!-- Receipt Instant Preview Modal -->
    <div id="receiptModal" class="modal">
        <div class="modal-content" style="max-width:420px; text-align:center;">
            <iframe id="receiptFrame" src="" style="width:100%; height:320px; border:none;"></iframe>
            <button onclick="document.getElementById('receiptModal').style.display='none'" class="btn-submit" style="background:#64748b; margin-top:10px;">Cufi (Close)</button>
        </div>
    </div>
    ...
    function openReceiptModal(txnId) {{
        document.getElementById('receiptFrame').src = "/receipt/" + txnId;
...
def approve_cust(cust_id):
    ...
    if c_info:
        ...
        send_notification(f"Maammila ({cust_id}) Manager/Auditor'n Approve godhameera.", session['username'])

    return redirect('/pending')

# --- 4. STATEMENT WITH PROPER RUNNING BALANCE CALCULATION ---
...
# --- 2. AUDITOR CLOSE WITH EXACT MAKER CASH SETTLEMENT CALCULATOR ---
...
def auditor_close():
    ...
    if request.method == 'POST':
        ...
        send_notification(f"Herregni guyyaa har'aa ({today_str}) AUDITOR'n CUFAMEERA.", session['username'])
    ...
    for m in maker_summary:
        net_cash_due = m['total_dep'] - m['total_with']
        settlement_str = f"<b style='color:#16a34a;'>+{net_cash_due:,.2f} Birr (Deebisuu/Galchuu Qaba)</b>" if net_cash_due >= 0 else f"<b style='color:#dc2626;'>{net_cash_due:,.2f} Birr (Fudhatuu Qaba)</b>"
        ...
        <div class="item-card" style="border-left:4px solid #ea580c;">
            ...
            <div style="font-size:12px; margin-top:8px; padding-top:6px; border-top:1px dashed #fdba74;">
                💵 Qarshii Hanga Deebisu/Galchu Qabu: {settlement_str}
            </div>
        </div>
        """
    ...
    <h3 style="font-size: 13px; color:#334155; margin-bottom:8px;">📊 To'annoo fi Shallaggii Qarshii Maker-oota Guyyaa Har'aa</h3>
